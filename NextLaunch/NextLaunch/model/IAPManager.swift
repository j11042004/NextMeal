...
@objc protocol IAPManagerDelegate {
    ...
    /// 購買的商品
    @objc func managerBoughtProduct(With productID : String)
    /// 告知所有商品資訊已取得
    @objc func managerGotAllProduct(With products : [SKProduct])
    /// 是否正在購買中
...
class IAPManager: NSObject {
    ...
    public func requestProductFromApple(WithProductIDs productsID : [String]){
        // 判斷是否可以進行交易
        guard SKPaymentQueue.canMakePayments() else {
            self.showSureAlert(Title: "錯誤", Message: "無法取得商品資訊，請與開發者聯絡!")
            return
        }
        let productIdSet : Set<String> = NSSet(array: productsID) as! Set<String>
    ...
    func paymentQueue(_ queue: SKPaymentQueue, updatedTransactions transactions: [SKPaymentTransaction]) {
        for transaction in transactions {
            ...
            case .restored:
                ...
                print("商品回覆購買")
                print("回復購買的商品 ID：\(transaction.payment.productIdentifier)")
                SKPaymentQueue.default().finishTransaction(transaction)
            ...
            case .failed :
                ...
                if let error = transaction.error {
                    print("交易失敗原因：\(error.localizedDescription)")
                }else{
                    print("不明原因")
                }
                // 結束交易
...
//            let payment = SKPayment(product: self.productsArray[self.selectProductIndex])
            ...
            payment.applicationUsername = "buy a new product"
